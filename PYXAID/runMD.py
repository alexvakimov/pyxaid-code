...
# This MD uses corrected NAC method
    ...
    while t<=stop_indx:
        ...
        if t==start_indx:
            # Run calculations
            os.system( "mpirun -n %s %s < %s.%d.in > %s.%d.out" % (NP,EXE,prefix0,t,prefix0,t) )
            os.system( "mpirun -n %s %s < x0.exp.in > x0.exp.out" % (NP,EXE_EXPORT) )
            ...
            if nac_method>=1:  # In addition compute the charged system
                os.system( "mpirun -n %s %s < %s.%d.in > %s.%d.out" % (NP,EXE,prefix1,t,prefix1,t) )
                os.system( "mpirun -n %s %s < x1.exp.in > x1.exp.out" % (NP,EXE_EXPORT) )
